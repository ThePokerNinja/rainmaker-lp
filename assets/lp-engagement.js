...
(function (global) {
  ...
  const FIRST_TOUCH_KEY = "rm_first_touch";
  ...
  function writeFirstTouch(record) {
    try {
      localStorage.setItem(FIRST_TOUCH_KEY, JSON.stringify(record));
    } catch {
      /* private mode - attribution degrades to per-visit */
    }
  }

  function inferFromReferrer(ref) {
    if (!ref) return null;
    let host = "";
    try {
    ...
    if (!host || host === global.location.hostname) return null;
    if (/(^|\.)linkedin\.com$|(^|\.)lnkd\.in$/.test(host)) return { utm_source: "linkedin", utm_medium: "referral" };
    if (/(^|\.)t\.co$|(^|\.)x\.com$|(^|\.)twitter\.com$/.test(host)) return { utm_source: "x", utm_medium: "referral" };
    if (/(^|\.)facebook\.com$|(^|\.)instagram\.com$|(^|\.)fb\.me$/.test(host)) return { utm_source: "meta", utm_medium: "referral" };
    if (/(^|\.)google\./.test(host)) return { utm_source: "google", utm_medium: "organic" };
    return { utm_source: host, utm_medium: "referral" };
  }

  /**
   * Resolve campaign attribution in priority order: query string, then the
   * stored first touch, then the referrer, then nothing. `basis` records which
   * rung we landed on so declared attribution stays distinguishable from
   * inferred. In-app browsers routinely strip the query string, which is what
   * previously made LinkedIn traffic look direct.
   */
  function resolveAttribution() {
    const referrer = global.document ? document.referrer || null : null;
    const landingPage = global.location.pathname;
    const query = parseUtm();
    const stored = readFirstTouch();

    if (Object.keys(query).length) {
      const record = { utm: query, referrer, basis: "query", landing_page: landingPage, landed_at: Date.now() };
      if (!stored) writeFirstTouch(record);
      return record;
    }
    if (stored && stored.utm && Object.keys(stored.utm).length) {
      return {
        utm: stored.utm,
        referrer,
        basis: "first_touch",
        landing_page: stored.landing_page || landingPage,
        landed_at: stored.landed_at || null,
      };
    }
    const inferred = inferFromReferrer(referrer);
    if (inferred) {
      const record = { utm: inferred, referrer, basis: "referrer", landing_page: landingPage, landed_at: Date.now() };
      if (!stored) writeFirstTouch(record);
      return record;
    }
    return { utm: {}, referrer, basis: "default", landing_page: landingPage, landed_at: Date.now() };
  }

  function lpSlug() {
    const q = new URLSearchParams(global.location.search);
    return q.get("lp") || q.get("utm_campaign") || "unknown";
  }
  ...
  function ensureSessionStart() {
    ...
    track("lp_session_start", {
      ...
      attribution_basis: state.attribution.basis,
  ...
  // First-party funnel events always run; GA4 alone is consent-gated in consent-banner.js.
  bootTracking();

  if (global.RM_CONSENT) {
    global.RM_CONSENT.init(function () {
      /* GA4 only */
    });
  }
